...
def forget_password():
    ...
    try:
        ...
        # Tận dụng dọn dẹp các OTP hết hạn khác trước khi tạo cái mới (Lazy Cleanup)
        try:
            clean_expired_otps()
        except Exception:
            pass
...
def reset_password():
    ...
    try:
        ...
        if update_user_password(email, new_password):
            ...
            # Tận dụng dọn dẹp các OTP hết hạn khác (Lazy Cleanup)
            try:
                clean_expired_otps()
            except Exception:
                pass
...
# Thay đổi để tương thích hoàn toàn với môi trường Vercel Serverless
